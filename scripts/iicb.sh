...
do
    ...
    echo "Введите группу,в которой Вы хотите  удалить пользователя(q - прекратить выполнение действия):"
    read groupname
    if [[ "$groupname" == "q" ]]
    then
	break
    ...
    if [[ "$groupname" == "help" ]]
    then
	echo "Вам необходимо ввести название группы,в которой вы хотите удалить пользователя"
    fi
